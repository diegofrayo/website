En este tutorial quiero explicar como subir un paquete privado de npm a Github Packages usando Github Actions. Además el paquete estará asociado a un repositorio privado perteneciente a una organización.
...
1. Configurar el `package.json` del paquete que quieres subir. Ejemplo:
  ...
  > **Importante:**
  > - `"name": "@my-organization/my-pkg"` | El nombre del paquete debe tener la estructura **@YOUR_ORGANIZATION/YOUR_PACKAGE_NAME**
...
1. Crear el archivo `.yml` del workflow en la carpeta `.github/workflows`.
  ...
  > **Importante:**
  > - Este workflow se encarga de instalar y construir el paquete usando npm a través de los steps `name: Install dependencies` y `name: Build package`.
