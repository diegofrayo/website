...
import { isConfirmAlertAccepted, showAlert } from "~/@legacy/src/utils/browser";
import { generateDate, getDatesDiff } from "~/@legacy/src/utils/dates";
import { sortBy } from "~/@legacy/src/utils/objects-and-arrays";
		...
		>
			<MainLayout title={`${PAGE_TITLE}`}>
				<Block className="tw-mx-auto tw-w-96 tw-max-w-full">
					...
					<audio
						src={`/@legacy/static/sounds/tick-${ticksAudio}.mp3`}
						id="audio-tick"
						preload="auto"
						className="tw-hidden"
					/>
					<audio
						src={`/@legacy/static/sounds/tick-${cyclesAudio}.mp3`}
						id="audio-cycle-completed"
						preload="auto"
						className="tw-hidden"
					/>
					<audio
						src="/@legacy/static/sounds/completed-3.mp3"
