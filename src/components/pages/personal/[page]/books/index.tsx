						...
						{books.map(({ id, title, calification, author, year, url, cover }) => {
							return (
								<Link
									...
									href={url}
									className="dfr-border-color-dark-strong tw-relative tw-mx-2 tw-mb-6 tw-h-64 tw-w-48 tw-overflow-hidden tw-rounded-br-md tw-rounded-tr-md tw-border-l-8 tw-shadow-lg tw-duration-500 hover:tw-translate-x-1 hover:tw--translate-y-1 hover:tw-rotate-0 hover:tw-opacity-75 hover:tw-shadow-2xl dark:dfr-border-color-secondary-inv sm:tw--rotate-1"
									isExternalLink
								>
									<Block
										is="article"
									...
									>
										<Icon
											wrapperClassName="tw-absolute tw--top-1 tw--right-1 dfr-bg-color-bw-inv dark:dfr-bg-color-wb-inv tw-rounded-md tw-shadow-md tw-p-2 tw-w-10 tw-h-10"
										...
										<Block className="tw-flex tw-w-full tw-flex-col tw-items-stretch tw-self-end tw-rounded-tr-lg tw-bg-opacity-70 tw-p-2 dfr-bg-color-wb-inv">
											<Title
												is="h1"
												variant={Title.variant.UNSTYLED}
												className="tw-mb-0.5 tw-break-normal tw-uppercase tw-leading-tight dfr-text-color-black-pin"
											>
												{title}
											</Title>
											...
											<Text className="tw-text-right tw-text-xs tw-font-bold tw-leading-none dfr-text-color-black-pin">
